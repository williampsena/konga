...
module.exports = {
  ...
  createCron: function (node) {
    ...
    return cron.schedule("* * * * *", function () {
      ...
      KongService.nodeStatus(node, function (err, data) {
        ...
        if (err) {
          if (!tasks[node.id].firstFailed)
            tasks[node.id].firstFailed = new Date();
          ...
          sails.log(
            "health_checks:cron:checkStatus => Health check for node " +
              node.id +
              " failed " +
              tasks[node.id].timesFailed +
              " times"
          ...
          var timeDiff = Utils.getMinutesDiff(
            new Date(),
            tasks[node.id].lastNotified
          );
          sails.log(
          ...
          if (!_.get(data, "database.reachable")) {
            ...
            if (!tasks[node.id].firstSucceeded)
              tasks[node.id].firstSucceeded = new Date();
            tasks[node.id].timesFailed = 0;
    ...
    sails.models.settings
      ...
      .exec(function (err, settings) {
        ...
        if (
          !settings.length ||
          !settings[0].data ||
          !settings[0].data.notify_when.node_down.active
        )
          return false;

        Utils.sendSlackNotification(
          settings[0],
          self.makePlainTextNotification(node)
        );
  ...
  makePlainTextNotification: function makePlainTextNotification(node) {
    var duration = moment
      .duration(moment().diff(moment(tasks[node.id].lastSucceeded)))
      .humanize();

    var text =
      "[ " +
      moment().format("MM/DD/YYYY @HH:mm:ss") +
      " ] A Kong Node  health check is failing for more than " +
      duration +
      ". ID: " +
      node.id +
      " | Name: " +
  ...
  makeHTMLNotification: function makeHTMLNotification(node) {
    ...
    var html =
      "<h3>A Kong Node health check is failing for more than " +
      duration +
      "</h3>" +
      "<h4>" +
      _.get(tasks, `${node.id}.lastFailedReason`, "N/A") +
