...
module.exports = {
  ...
  createCron: function (hc) {
    ...
    return cron.schedule("* * * * *", async () => {
      ...
      try {
        ...
        if (!connection)
          return sails.log("Upstream health => No connection set. Ending it.");
      ...
      .exec(function (err, transport) {
        ...
        switch (settings.data.default_transport) {
          ...
          case "mailgun":
            ...
            if (
              !_.get(transport, "settings.auth.api_key") ||
              !_.get(transport, "settings.auth.domain")
            ) {
  ...
  notify: function (hc, connection, unhealthyTargets) {
    ...
    if (!hc.email && !hc.slack) {
      return sails.log(
        "Slack and email notifications are disabled for this Upstream."
      );
    }
      ...
      .exec(function (err, settings) {
        ...
        if (hc.slack) {
          Utils.sendSlackNotification(
            settings[0],
            self.makePlainTextNotification(connection, unhealthyTargets)
          );
        } else {
        ...
        if (hc.email) {
          self.createTransporter(settings[0], function (err, result) {
            if (err || !result || !result.transporter) {
              ...
              var html = self.makeHTMLNotification(
                connection,
                unhealthyTargets
              );
  ...
  notifyNotificationEndpoint: function (hc) {
    ...
    if (!hc.notification_endpoint) {
      sails.log(
        "api_Upstream health:notifyNotificationEndpoint => No notification endpoint defined"
      );
      return false;
    }
    ...
    unirest
      ...
      .end(function (response) {
        if (response.error) {
          sails.log(
            "api_Upstream health:notifyNotificationEndpoint => Failed to notify notification endpoint",
            response.error
          );
        } else {
          sails.log(
            "api_Upstream health:notifyNotificationEndpoint => Succeeded to notify notification endpoint"
          );
        }
      });
  },

  makeHTMLNotification: function makeHTMLNotification(
    connection,
    unhealthyTargets
  ) {
    let html =
      "<h2>Some Upstream health checks have failed</h2>" +
      "<h3>Connection</h3>" +
  ...
  makePlainTextNotification: function (connection, unhealthyTargets) {
    var text =
      ...
      " ] Some upstream health checks have failed: ";
    unhealthyTargets.forEach((target) => {
      text +=
        "```Connection: `" +
        connection.name +
